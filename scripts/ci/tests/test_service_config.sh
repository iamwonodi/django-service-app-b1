...
param $NAME "$(jq '.database.port = 3306 | .database.port_parameter = null | .database.host = "core-production-mysql.x.rds.amazonaws.com"' <<< "$CFG")"; : > "${FAKE_LOG}"
out="$(run 2>/dev/null)"
check "a managed database's published port is used"      bash -c "[ \"$(val DATABASE_PORT)\" = 3306 ] && [ \"$(val DATABASE_HOST)\" = core-production-mysql.x.rds.amazonaws.com ]"
check "and no port parameter is read for it"             bash -c "! grep -q 'engines/postgres/port' '${FAKE_LOG}'"
param $NAME "$(jq '.database.port = null | .database.port_parameter = null' <<< "$CFG")"
check "a database with no port at all is refused"        bash -c "! bash '$R' acme auth eu-west-1 >/dev/null 2>&1"
param $NAME "$(jq '.database.port = "5432; rm"' <<< "$CFG")"
check "a published port that is not a number is refused" bash -c "! bash '$R' acme auth eu-west-1 >/dev/null 2>&1"
param $NAME 'not json'
